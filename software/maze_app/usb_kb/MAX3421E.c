...
BYTE SPI_wr(BYTE data) {
	...
	int returnCode;
	BYTE status;
	returnCode = alt_avalon_spi_command(SPI_0_BASE,0,1,&data,1,&status,0);
	if(returnCode < 0)
	{
		printf("Error for SPI_wr because returnCode < 0 = %x \n", returnCode);
	}
	return status;
}
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE combineData[2] = {reg+2,val};
	returncode = alt_avalon_spi_command(SPI_0_BASE,0,2,combineData,0,&readData,0);

	if(returncode < 0)
	{
		printf("Error for MAXreg_wr because returnCode < 0 = %x \n", returncode);	
	}
	return;
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	returncode = alt_avalon_spi_command(SPI_0_BASE,0,nbytes+1,input,0,&read_data,0);

		if(returncode < 0)
	{
		printf("Error for MAXbytes_wr because returnCode < 0 = %x \n", returncode);	
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	if(return_code < 0)
	{
		printf("Error for MAXreg_rd because returnCode < 0 = %x \n", return_code);	
	}
	return val;
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if(return_CODE < 0)
	{
		printf("Error for MAXbytes_rd because returnCode < 0 = %x \n", return_CODE);
	}
